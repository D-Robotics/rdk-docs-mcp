...
3. 先 `get_page` 打开手册命中。手册已经能回答时，不要再开论坛、也不要在答案里并列一篇社区帖。
4. 只有手册没写、对不上版本，或用户明确问社区怎么说时，再 `source: "forum"` / `list_toc(manual=forum)`，最多补 1 篇帖子。不要 WebFetch 论坛 HTML。
5. 回答必须带可点击链接。以手册步骤、接口、版本为准。手册没写、论坛也没写，就说没写，不要编。
6. S 系列 OE / S100·S600 OE LLM（`oe-s`、`oe-llm-s100`、`oe-llm-s600`）按普通手册搜即可。它们是 Rspress CSR，页面 HTML 是空壳，`get_page` 会从站点自带的 `search_index` 还原正文。
